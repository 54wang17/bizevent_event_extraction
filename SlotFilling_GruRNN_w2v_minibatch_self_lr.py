from model.slot_filling.gru_rnn_minibatch_w2v_self_lr import GruRNN
import os
import cPickle
import numpy as np
import theano
...
model_dir = os.path.join(root_dir, 'model', 'slot_filling', 'GruMiniBatchW2VUp_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch))
if not os.path.exists(model_dir):
    os.mkdir(model_dir)

# initialize model
model = GruRNN(in_dim=in_dim, out_dim=out_dim, hidden_dim=hidden_dim, word_embedding=W2V)
