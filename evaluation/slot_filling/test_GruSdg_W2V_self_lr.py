from bizevent_event_extraction.model.slot_filling.gru_rnn_w2v_self_lr import GruRNN
import cPickle
...
model_dir = os.path.join(root_dir.replace('evaluation', 'model'), 'GruSdgW2VUp_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch))
test_file = open('GruSdgW2VUp_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch),'w+')
...
for root, dirs, files in os.walk(model_dir, topdown=False):
    ...
    for file in files:
        ...
        rnn = GruRNN(in_dim=in_dim, hidden_dim=hidden_dim, out_dim=out_dim, word_emb=W2V)
        rnn.load_model_parameters(model_file)
        rnn.build_model()


        predictions_test = [map(lambda x: idx2label[x], rnn.predict(x)) for x in test_x]
        groundtruth_test = [map(lambda x: idx2label[x], y) for y in test_Y]
        words_test = [map(lambda x: idx2word[x], w) for w in test_X]

        predictions_valid = [map(lambda x: idx2label[x], rnn.predict(x)) for x in valid_x]
