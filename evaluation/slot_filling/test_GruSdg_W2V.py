from bizevent_event_extraction.model.slot_filling.gru_rnn_w2v import GruRNN
import cPickle
...
model_dir = os.path.join(root_dir.replace('evaluation', 'model'), 'GruSdgW2V_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch))
test_file = open('GruSdgW2V_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch),'w+')
