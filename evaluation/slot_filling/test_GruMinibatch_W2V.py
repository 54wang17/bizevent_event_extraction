from bizevent_event_extraction.model.slot_filling.gru_rnn_minibatch_w2v import GruRNN
import cPickle
...
model_dir = os.path.join(root_dir.replace('evaluation', 'model'), 'GruMiniBatchW2V_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch))
test_file = open('GruMiniBatchW2V_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch),'w+')
...
for root, dirs, files in os.walk(model_dir, topdown=False):
    ...
    for file in files:
        ...
        predictions_valid = [map(lambda x: idx2label[x], y) for y in rnn.get_prediction(test_x)]
