from model.slot_filling.gru_rnn_minibatch_rd_self_lr import GruRNN
import os
import cPickle
import numpy as np
import theano
...
_BATCH_SIZE = int(os.environ.get('BATCH_SIZE', '25'))
CLASSIFIER_NUM = {'IPO': 20, 'Layoff': 2}
ROOT_DIR = {'IPO': '', 'Layoff': './layoff'}
...
model_dir = os.path.join(root_dir, 'model', 'slot_filling', 'GruMiniBatchRdUp_%s_%f_%d_%d' % (data_group, lr, batch_size, nepoch))
if not os.path.exists(model_dir):
    os.mkdir(model_dir)

# initialize model
model = GruRNN(in_dim=in_dim, out_dim=out_dim, hidden_dim=hidden_dim, word_size=W2V.shape[0])
# Begin training model with mini batch
model.train_with_mini_batch(data_group, train_x, train_Y, valid_x, valid_Y,
